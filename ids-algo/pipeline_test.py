...
async def run_ids_pipeline(text_content: str, return_timing: bool = False, return_stage_outputs: bool = False) -> Tuple[Dict[str, Any], Dict[str, float], Dict[str, Any]]:
    ...
    try:
        ...
        for facet in facets:
            ...
            # Include ifc_item if present (contains full IFC definition)
            if hasattr(facet, 'ifc_item') and facet.ifc_item:
                # Convert ifc_item to dict if it has to_dict method
                if hasattr(facet.ifc_item, 'to_dict'):
                    mapping["ifc_item"] = facet.ifc_item.to_dict()
                else:
                    # Fallback: extract key attributes manually
                    mapping["ifc_item"] = {
                        "name": getattr(facet.ifc_item, 'name', ''),
                        "definition": getattr(facet.ifc_item, 'definition', ''),
                        "ifc_version": getattr(facet.ifc_item, 'ifc_version', ''),
                        "item_type": getattr(facet.ifc_item, 'item_type', ''),
                    }
            else:
