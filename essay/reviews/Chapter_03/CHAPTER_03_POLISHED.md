...
**性能要求**。参考同类BIM自动化审查系统的实现经验[55][57]，系统设定以下性能目标：单次IDS生成响应时间低于10秒（含LLM调用与知识库检索）；100MB以内的IFC模型审查时间低于30秒；支持至少10个并发用户正常访问。其中，10秒IDS生成响应的构成为：自然语言结构化解析约2秒、方面分类约1秒、向量检索与知识图谱验证约3秒、约束提取约2秒、IDS JSON格式化与验证约2秒。30秒IFC审查的设定依据为：ifctester引擎解析100MB IFC文件约需8-12秒，执行2,000条IDS规则匹配约需15-20秒，报告格式化约需3-5秒。
...
数据层采用MongoDB数据库存储用户数据、历史记录与审查报告等持久化信息；MongoDB GridFS（Grid File System，网格文件系统）用于处理大文件存储；部分静态资源通过UploadThing服务托管。
...
**MongoDB数据库**。MongoDB是文档型数据库，其灵活的文档结构适合存储不规则的BIM数据。前文已述，Ellul等[54]的对比实验为MongoDB在BIM场景中的应用提供了实证支撑。本研究的IDS JSON与审查报告等层次化数据可直接映射为MongoDB文档，无需预定义严格表结构。

**GridFS机制**。GridFS是MongoDB的大文件存储方案，将文件分块存储于chunks与files两个集合，突破单文档16MB限制[58]。BIM模型文件通常数十MB，GridFS的分块存储便于文件的流式传输与部分读取。
