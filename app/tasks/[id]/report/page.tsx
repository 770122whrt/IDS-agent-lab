...
export default function ReportPage() {
    ...
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600 dark:text-slate-400">加载报告中...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 text-center">
            ...
            <p className="text-red-600 dark:text-red-400 font-medium mb-4">{error || "任务不存在"}</p>
            <button
  ...
  const checkedAt = task.checkedAt ? new Date(task.checkedAt).toLocaleString("zh-CN") : "未知";
  ...
  if (!reportData || !reportData.specifications) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6">
        <div className="max-w-2xl mx-auto">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 text-center">
            <p className="text-slate-600 dark:text-slate-400 mb-4">报告数据不存在或格式错误</p>
            <button
              onClick={() => router.push("/tasks")}
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4">
            <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1">规格总数</p>
            <p className="text-2xl font-semibold text-slate-900 dark:text-white">{total_specifications}</p>
          </div>
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4">
            <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1">通过规格</p>
            <p className="text-2xl font-semibold text-emerald-600 dark:text-emerald-400">{total_specifications_pass}</p>
          </div>
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4">
            <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1">检查项</p>
            <p className="text-2xl font-semibold text-slate-900 dark:text-white">{total_checks}</p>
          </div>
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4">
            <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1">失败项</p>
...
function SpecificationCard({ spec, specIndex, expandedReqs, onToggleRequirement }: SpecificationCardProps) {
    ...
    <div className={`bg-white dark:bg-slate-900 rounded-xl border overflow-hidden transition-shadow hover:shadow-md ${
      ...
      <div className="p-5 border-b border-slate-100 dark:border-slate-800">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium ${
              ...
              {spec.cardinality === "required" ? "必须" : "可选"}: {spec.status ? "通过" : "失败"}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-slate-600 dark:text-slate-400">
          <span>检查: <strong className="text-slate-900 dark:text-white">{spec.total_checks_pass}</strong>/{spec.total_checks} 通过</span>
          <span>构件: <strong className="text-slate-900 dark:text-white">{spec.total_applicable_pass}</strong>/{spec.total_applicable} 通过</span>
          {hasFailures && (
            <span className="text-red-600 dark:text-red-400">
              <strong>{spec.total_checks_fail}</strong> 个失败
            </span>
          )}
        </div>
      ...
      <div className="px-5 py-4 bg-slate-50/50 dark:bg-slate-800/30 border-b border-slate-100 dark:border-slate-800">
        ...
        {spec.applicability?.length > 0 ? (
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500">无适用范围</p>
        )}
      </div>
      ...
      <div className="p-5">
        ...
        {spec.requirements?.length > 0 ? (
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500">无要求</p>
        )}
      </div>
    </div>
  );
...
function RequirementItem({ requirement: req, reqIndex, isExpanded, onToggle }: RequirementItemProps) {
            ...
            <div className="overflow-x-auto rounded-lg border border-red-100 dark:border-red-900/30">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-red-50 dark:bg-red-900/20">
                    <th className="px-3 py-2 text-left font-medium text-red-700 dark:text-red-300">类型</th>
                    <th className="px-3 py-2 text-left font-medium text-red-700 dark:text-red-300">名称</th>
                    <th className="px-3 py-2 text-left font-medium text-red-700 dark:text-red-300">GlobalId</th>
                    <th className="px-3 py-2 text-left font-medium text-red-700 dark:text-red-300">错误原因</th>
                  </tr>
                </thead>
              ...
              {failedCount > 5 && (
                <div className="px-3 py-2 bg-slate-50 dark:bg-slate-800 text-xs text-slate-500 dark:text-slate-400 border-t border-red-100 dark:border-red-900/20">
                  + 还有 {failedCount - 5} 个构件出现相同错误 (已省略)
                </div>
              )}
            </div>
          ) : req.status ? (
